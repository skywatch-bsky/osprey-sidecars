...
class TestRunCycle:
    ...
    def test_death_events_skipped_in_writes(self, app_config: AppConfig) -> None:
        ...
        and a surviving cluster today. Verify:
        (1) The death cluster_id is ABSENT from captured cluster inserts.
        (2) The surviving cluster's id IS present in captured cluster inserts.
        (3) Surviving cluster membership IS present.
        (4) Mutation test: if the death-skip filter is removed, the test must FAIL.

        The death events are generated by compute_evolution but filtered by
        `if event.evolution_type != 'death'` before insertion. The test verifies
        this filter works by checking death cluster IDs are absent from inserts.
        """
        fake_db = FakeDb()
        ...
        # This is the core of the test - if the death-skip filter is removed,
        # the death event might appear here (depending on implementation),
        # or it might not due to the zip filtering in the code.
