...
from quote_overdispersion.analyzer import score_rows
from quote_overdispersion.config import AppConfig
from quote_overdispersion.db import QuoteOverdispersionDb
from quote_overdispersion.queries import (
    daily_aggregation_query,
    hourly_aggregation_query,
)
from quote_overdispersion.telemetry import (
    TelemetryHandles,
    noop_telemetry,
    record_failure,
    record_run_metrics,
...
logger = logging.getLogger('quote_overdispersion')
...
def run_cycle(db: QuoteOverdispersionDb, config: AppConfig, telemetry: TelemetryHandles | None = None) -> None:
    run_timestamp = datetime.now(timezone.utc)
    telemetry = telemetry or noop_telemetry()

    with telemetry.tracer.start_as_current_span(
        'quote_overdispersion.run_cycle',
        ...
        try:
            ...
            for granularity, query_fn in [
                ...
                with stage_span(telemetry, f'quote_overdispersion.{granularity}.cycle', granularity=granularity):
                    logger.info(f'running {granularity} aggregation query')
                    with stage_span(
                        telemetry,
                        'quote_overdispersion.fetch_aggregated_rows',
                    ...
                    with stage_span(telemetry, 'quote_overdispersion.score_rows', granularity=granularity):
                    ...
                    with stage_span(telemetry, 'quote_overdispersion.insert_results', granularity=granularity):
...
def main() -> None:
    ...
    logger.info(f'starting quote-overdispersion detector (interval={config.analysis.interval_seconds}s)')
    logger.info(
        f'volume threshold={config.analysis.volume_p_threshold}, density threshold={config.analysis.density_p_threshold}'
    )

    db = QuoteOverdispersionDb(config.clickhouse)
