...
Detects anomalous domain sharing patterns using dispersion-aware statistical tests and false-discovery-rate control. Reads from `osprey_execution_results`, writes scored results to `url_overdispersion_results` with volume and density anomaly scores.
...
- **Output:** `url_overdispersion_results` (scored domains per time bucket with volume and density anomaly scores, q-values, and diagnostic columns)
...
### Volume Testing
- **Baseline:** Median daily share count (rolling 14-day window, densified with zeros)
...
- **Population baseline:** Used when entity baseline unavailable; computed over today's scored domains in the same batch
...
- **Population scope:** Population medians computed only over today's scored entities (≥ 3 unique sharers)

### Baseline Densification
- **Dense grid:** All (domain, day) pairs from first-seen through today, with zero volume/NULL density for inactive days
...
- **Window sampling:** arraySlice to first 5 sample user IDs and first 5 sample URLs per (domain, bucket)
